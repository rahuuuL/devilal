...
public class SharedBatchSourceRegistry {
	...
	private List<?> callSource(String sourceProviderCode, Map<String, Object> resolvedParams) {
		if ("CONSISTENT_VOLUME_SOURCE".equals(sourceProviderCode)) {
			...
			return detector.detectConsistentVolumes(tickers, fromDate, toDate, baselineWindow, baselineLowPercentile,
					baselineHighPercentile, rvolPercentileWindow, rvolThresholdPercentile, consistencyWindow,
					requiredScore);
		}
		return List.of();
	}
	...
	private String paramsKey(Map<String, Object> resolvedParams) {
		...
		return resolvedParams.entrySet().stream().sorted(Map.Entry.comparingByKey())
