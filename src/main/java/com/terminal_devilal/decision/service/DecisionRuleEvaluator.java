...
public class DecisionRuleEvaluator {
	...
	public List<String> evaluateAndReturnFiredRules(List<RuleDefinition> rules, SubjectContext context,
			DecisionState state) {
		...
		rules.stream().sorted(Comparator.comparingInt(RuleDefinition::priority).reversed()).forEach(rule -> {
			...
			if (allConditionsMatched) {
				...
				log.info("Rule {} fired for subject {} {}. Applying {} action(s)", rule.name(),
						context.getSubjectType(), context.getSubjectId(), rule.actions().size());
				rule.actions().forEach(action -> apply(action, context, state));
			}
		});
		log.info("Completed rule evaluation for subject {} {}. Fired rules: {}", context.getSubjectType(),
				context.getSubjectId(), fired);
		return fired;
	}
	...
	private boolean matches(RuleDefinition.Condition condition, SubjectContext context) {
		...
		if (!(actual instanceof Number) || !(condition.value() instanceof Number)) {
			...
			log.debug("Condition {} non-numeric compare result={} actual={} expected={}", condition.indicator(), result,
		...
		log.debug("Condition {} numeric compare result={} left={} right={}", condition.indicator(), result, left,
				right);
		return result;
	}
	...
	private void apply(RuleDefinition.Action action, SubjectContext context, DecisionState state) {
		if (action.expression() instanceof RuleDefinition.Literal literal) {
			if (action.type() == RuleDefinition.ActionType.SET || action.type() == RuleDefinition.ActionType.TAG) {
				log.info("Applying literal action target={} type={} value={}", action.target(), action.type(),
						literal.value());
				state.set(action.target(), literal.value());
			}
			return;
		}
		BigDecimal value = action.expression() == null ? action.constantValue()
				: evaluate(action.expression(), context);
		log.info("Applying action target={} type={} computedValue={} constantValue={}", action.target(), action.type(),
				value, action.constantValue());
		switch (action.type()) {
	...
	private BigDecimal evaluate(RuleDefinition.Expression expression, SubjectContext context) {
		if (expression instanceof RuleDefinition.Constant constant)
			return constant.value();
		if (expression instanceof RuleDefinition.Attribute attribute)
			return number(context.getAttribute(attribute.code()));
		if (expression instanceof RuleDefinition.Literal literal)
			throw new IllegalArgumentException("Literal expressions are only valid for SET or TAG actions");
		if (expression instanceof RuleDefinition.BinaryExpression binary) {
			...
			case DIVIDE -> left.divide(right, 12, RoundingMode.HALF_UP);
	...
	private BigDecimal number(Object value) {
		if (!(value instanceof Number))
			throw new IllegalArgumentException("Expression attribute is not numeric");
